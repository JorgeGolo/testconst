...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      // Lógica para obtener los datos de Firestore
      ...
      // Llamada a OpenAI para procesar la información obtenida
      const openAIResponse = await openai.completions.create({
        model: 'text-davinci-003',
        prompt: `Genera una breve descripción o resumen del artículo ${randomArticulo} de la Constitución, que contiene: ${contenido}`,
        max_tokens: 150,
      });

      const openAIText = openAIResponse.choices[0].text.trim();

      // Respuesta al cliente con datos y el resultado de OpenAI
      res.status(200).json({
        titulo: randomTitulo,
        ...
        contenido,
        openAIText,  // Agregar el resultado de OpenAI
      });
    } catch (error) {
